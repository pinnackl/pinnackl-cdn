...
class Cdn
{
    ...
    public function avatarAction(\Silex\Application $app, Request $request)
    {
        $success = array();
        $error = array();
        ...
        $imagePath = realpath(__DIR__.'/../../../web/avatars');
        $target_dir = $imagePath . "/";
        $target_file = $target_dir . basename($request->files->get('avatar')->getClientOriginalName());
        $uploadOk = 1;
        $imageFileType = pathinfo($target_file, PATHINFO_EXTENSION);
        // Check if image file is a actual image or fake image
        $check = $request->files->get('avatar')->getSize();
        if($check !== false) {
            // echo "File is an image - " . $check["mime"] . ".";
            $uploadOk = 1;
        } else {
        ...
        // Check file size
        if ($request->files->get('avatar')->getSize() > 21000000) {
            $error[] = "Sorry, your file is too large.";
            $uploadOk = 0;
        }
        // Allow certain file formats
        ...
        // if everything is ok, try to upload file
            ...
            $newName = uniqid(). "." . $imageFileType;
            if (move_uploaded_file($request->files->get('avatar')->getpathName(), $target_dir . $newName)) {
                $success[] = "The file ". basename($newName). " has been uploaded.";
            } else {
                $error[] = "Sorry, there was an error uploading your file.";
            }
